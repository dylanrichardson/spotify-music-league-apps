...
import { playerManager } from '../../../shared/player';
import { useToast } from '../../../shared/toast';
import { PlaybackBar } from '../../../shared/PlaybackBar';
...
// Extended track type with follower info
interface TrackWithFollowers extends SpotifyTrack {
...
export function Dashboard() {
  ...
  const [sortBy, setSortBy] = useState<'followers' | 'artist' | 'name' | 'album'>('followers');
  ...
  const handleSort = (field: 'followers' | 'artist' | 'name' | 'album') => {
    if (sortBy === field) {
  ...
  const getSortedTracks = (tracks: TrackWithFollowers[]) => {
    const sorted = [...tracks];
    const multiplier = sortDirection === 'asc' ? 1 : -1;

    switch (sortBy) {
      case 'followers':
        return sorted.sort((a, b) => ((a.followerCount || 0) - (b.followerCount || 0)) * multiplier);
      case 'artist':
  ...
  const handleFilter = async () => {
    if (!maxFollowers) {
      setError('Please enter max followers');
      return;
    }

    const maxFollowersNum = parseInt(maxFollowers);
    if (isNaN(maxFollowersNum) || maxFollowersNum <= 0) {
      setError('Please enter a valid number of followers');
      return;
    }
    ...
    try {
      // First, fetch all tracks
      let allTracks: SpotifyTrack[] = [];

      const collectTracks = (current: number, total: number, newTracks: SpotifyTrack[]) => {
      ...
      // Sort by follower count (ascending)
      tracksWithFollowers.sort((a, b) => (a.followerCount || 0) - (b.followerCount || 0));
      setFilteredTracks(tracksWithFollowers);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-400 via-pink-500 to-red-500">
      <div className="container mx-auto px-4 py-4 md:py-8 pb-24 md:pb-32">
        {/* Header */}
          ...
          <div className="flex flex-col gap-4 md:gap-6 mb-4 md:mb-6 max-w-2xl mx-auto">
            <div className="flex-1">
              ...
              <p className="text-xs text-gray-500 mt-2 text-center">
                Currently: {formatFollowers(parseInt(maxFollowers || '0'))} followers
              </p>
            </div>
          </div>
          ...
          >
            {loading ? (
              <span className="flex items-center justify-center gap-2">
                ...
                {loadingLibrary ? 'Loading library...' : 'Finding hidden gems...'}
              </span>
            ) : (
              'Find Hidden Gems'
            )}
            ...
            <div className="flex flex-col gap-3 mb-4 md:mb-6">
              <div className="flex items-center justify-between gap-3">
                <h2 className="text-lg md:text-2xl font-bold text-gray-800 flex items-center gap-2">
                  <span>Found {filteredTracks.length} hidden gems</span>
                  {loading && (
                    ...
                    <div key={track.id} className="border-b border-gray-100 last:border-0">
                      <div className="flex items-center gap-3 py-3 px-2">
                        ...
                        >
                          ...
                          <div className="text-xs text-gray-600 font-medium flex-shrink-0">
                            👥 {formatFollowers(track.followerCount)}
                          </div>
                        </button>
                      ...
                      {expandedTrack === track.id && (
                        <div className="px-2 pb-3 pt-0 bg-gray-50">
                          ...
                          <div className="text-xs text-gray-600 mb-2">
                            <span className="font-semibold">Artist Followers:</span> {formatFollowers(track.followerCount)}
                          </div>
                          <a
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-gray-200">
                        ...
                        <th
                          className="text-left py-3 px-4 text-sm font-semibold text-gray-700 w-32 cursor-pointer hover:bg-gray-50 select-none"
                          onClick={() => handleSort('followers')}
                        >
                          <div className="flex items-center gap-1">
                            Followers
                            {sortBy === 'followers' && <span className="text-blue-600">{sortDirection === 'asc' ? '↑' : '↓'}</span>}
                          </div>
                        </th>
                    ...
                    <div className="flex gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-500 font-medium">
                          👥 {formatFollowers(track.followerCount)} followers
                        </p>
                        <a
                          href={`https://open.spotify.com/track/${track.id}`}
                          target="_blank"
        ...
        {!loading && filteredTracks.length === 0 && progress.total > 0 && (
          <div className="bg-white rounded-lg shadow-xl p-6 md:p-8 text-center">
            <p className="text-gray-600 text-base md:text-lg">
              No songs found from artists with under {formatFollowers(parseInt(maxFollowers || '0'))} followers in your{' '}
